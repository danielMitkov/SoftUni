...
for(int row = 0;row<size;row++) {
    ...
    for(int col = 0;col<size;col++) matrix[row,col]=chars[col];
}
...
int CountAttackedKnights(int row,int col) {
    ...
    //horizontal left-up
    if(IsCellValid(row-1,col-2)) if(matrix[row-1,col-2]=='K') attackedKnights++;
    ...
    if(IsCellValid(row+1,col+2)) if(matrix[row+1,col+2]=='K') attackedKnights++;
    //vertical up-left
    if(IsCellValid(row-2,col-1)) if(matrix[row-2,col-1]=='K') attackedKnights++;
    //vertical up-right
    if(IsCellValid(row-2,col+1)) if(matrix[row-2,col+1]=='K') attackedKnights++;
    //vertical down-left
    if(IsCellValid(row+2,col-1)) if(matrix[row+2,col-1]=='K') attackedKnights++;
